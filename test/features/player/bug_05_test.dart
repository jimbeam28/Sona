...
void main() {
  ...
  group('BUG-05-T01: hanging task times out and gate resets', () {
    test('task that never completes triggers 20s timeout, gate accepts new request',
        () {
      FakeAsync().run((async) {
  ...
  group('BUG-05-T03: storage read hang caught by gate timeout', () {
    test('task simulating storage hang triggers timeout, returns failed-like error',
        () {
      FakeAsync().run((async) {
        final gate = SerializedRequestGate();
    ...
    test('task with internal 5s timeout completes before gate 20s timeout', () {
      FakeAsync().run((async) {
        ...
        expect(result, equals('storage_timeout_failed'),
            reason: '5s storage timeout should trigger before 20s gate timeout');
      });
    });
  });
  ...
  group('BUG-05-T05: consecutive failures each reset gate, 4th request succeeds', () {
      ...
      FakeAsync().run((async) {
        ...
        for (int i = 0; i < 3; i++) {
          ...
          gate.schedule<String>(
            ...
            onSuperseded: () => 's',
          ).catchError((_) { errorCaught = true; return 's'; });

          async.elapse(Duration.zero);
          expect(errorCaught, isTrue,
              reason: 'task $i should fail with error');
        }
