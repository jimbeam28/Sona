...
> 基于代码模式扫描发现。按严重性分组，建议在重构过程中顺带修复。

### 🔴 HANG 类 — await 无超时导致永久阻塞

| # | 位置 | 代码 | 风险 | 影响 |
|---|------|------|------|------|
| H-1 | `audio_handler.dart:200` | `await _player.play()` | 平台调用挂起 | 通知栏播放按钮无响应 |
| H-2 | `audio_handler.dart:214,241` | `await _player.stop()` | 平台调用挂起 | 通知栏停止按钮无响应；前台服务无法停止 |
| H-3 | `audio_handler.dart:207` | `await _player.pause()` | 平台调用挂起 | 通知栏暂停按钮无响应 |
| H-4 | `browser_provider.dart:427` | `await player.setAudioSource()` 启动恢复 | NAS 不可达时挂起 | **App 启动卡在加载页** |
| H-5 | `browser_provider.dart:429` | `await player.seek()` 启动恢复 | 平台调用挂起 | **App 启动卡在加载页** |
| H-6 | 5 处 `storage.read()` | `await storage.read(key: ...)` | SecureStorage 平台调用挂起 | 加载/验证/保存流程冻结 |
| H-7 | `connection_provider.dart:203,250` | `await storage.write()` | SecureStorage 挂起 | "保存中..." 永远转圈 |
| H-8 | `connection_provider.dart:289` | `await storage.delete()` | SecureStorage 挂起 | 删除操作卡住 |
| H-9 | `player_provider.dart:868,1004` | `await player.stop()` | 平台调用挂起 | 队列移除/加载失败处理卡住 |
| H-10 | `player_provider.dart:972` | `await player.setSpeed()` | 平台调用挂起 | loadAndPlay 流程卡住 |

**统一修复方案**：
- just_audio 的 `play()`/`stop()`/`pause()`/`setSpeed()`：添加 5 秒超时，超时后静默忽略（fire-and-forget 模式）
- `setAudioSource()`/`seek()`：添加 10 秒超时
- `SecureStorage.read()`/`write()`/`delete()`：创建 `safeStorageRead()` 工具函数，统一 5 秒超时
- 已有的 Bug 5（gate 超时）覆盖了 H-6~H-10 中在 `loadAndPlayProvider` 内部的调用

### 🔴 CRASH 类 — 异常未捕获或空指针

| # | 位置 | 代码 | 风险 | 影响 |
|---|------|------|------|------|
| C-1 | `connection_list_screen.dart:263,289,299,306` | `conn.id!` | DB 记录损坏时 id 为 null | **App 闪退** |
| C-2 | `playlist_detail_screen.dart:159,295` | `track.id!` | DB 记录未正确插入 | **App 闪退** |
| C-3 | `progress_provider.dart:75` | `upsertProgressProvider` 无 try-catch | DB 磁盘满/损坏 | **App 闪退**（定时器回调中调用） |
| C-4 | `progress_provider.dart:103` | `clearProgressProvider` 无 try-catch | DB 磁盘满/损坏 | **App 闪退** |
| C-5 | `player_screen.dart:482-483` | `Navigator.pop()` + `pushNamed` 双导航 | 路由栈异常时崩溃 | **App 闪退** |

**统一修复方案**：
- C-1/C-2：添加 `if (id == null) return;` 守卫
- C-3/C-4：在 provider 内部用 try-catch 包裹 DAO 调用，错误时日志记录但不崩溃
- C-5：合并为单次 `context.go('/connection')` 导航

### 🟡 LOW 类 — 边缘情况但不太可能触发

| # | 位置 | 说明 |
|---|------|------|
| L-1 | `browser_screen.dart:123` | `GoRouter.of(context)` 在对话框 await 后，理论上 context 可能失效（实际很难触发） |
| L-2 | 多处 fire-and-forget 调用 | `player.seek()`/`play()`/`pause()` 无 await，异常由 Zone 处理（标准模式，可接受） |
| L-3 | `connection_provider.dart:250` | `storage.write()` 无 try-catch，但调用方有（低风险） |
