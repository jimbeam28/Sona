...
├── core/                           # 基础设施层
│   ├── database/
│   │   ├── database_helper.dart    # SQLite 初始化与迁移（v1）
│   │   └── dao/
...
│       └── play_queue.dart         # 播放队列模型（含 PlayMode 枚举）
...
dependencies:
  ...
  flutter_slidable: ^4.0.3      # 滑动操作（连接列表）
