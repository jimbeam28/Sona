...
│       │            │            │            │                       │
├───────┼────────────┼────────────┼────────────┼───────────────────────┤
│       Domain Layer (Pure Dart — 零 Flutter/Riverpod 依赖)            │
│  ┌──────────────┐ ┌──────────────┐ ┌─────────────────┐              │
│  │PlaybackOrch- │ │DirectorySvc  │ │ ConnectionSvc   │              │
│  │  estrator    │ │CachePolicy   │ │ ConnValidator   │              │
│  │PlayMode      │ │NavigationStack│ │ StorageUtils   │              │
│  │SeekUtils     │ │SortOption    │ │                 │              │
│  │SpeedManager  │ │              │ │                 │              │
│  │MediaControl  │ │              │ │                 │              │
│  │BackgroundPB  │ │              │ │                 │              │
│  │RequestGate   │ │              │ │                 │              │
│  ├──────────────┤ ├──────────────┤ ├─────────────────┤              │
│  │PlaylistSvc   │ │ProgressSvc   │ │ SettingsSvc     │              │
│  │              │ │ProgressPolicy│ │ TimerService    │              │
│  └──────────────┘ └──────────────┘ └─────────────────┘              │
│       │            │            │            │                       │
├───────┼────────────┼────────────┼────────────┼───────────────────────┤
│       Contracts Layer (抽象接口)                                      │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐ ┌────────────┐  │
│  │IAudioPlayer  │ │IAudioHandler │ │IConnectionDao│ │ISecureStor.│  │
│  │              │ │              │ │IProgressDao  │ │            │  │
│  │              │ │              │ │IPlaylistDao  │ │            │  │
│  └──────────────┘ └──────────────┘ └──────────────┘ └────────────┘  │
│       │            │            │            │                       │
...
│                       Service Layer                                  │
│  ┌──────────────┐ ┌──────────────┐ ┌─────────────────┐              │
│  │WebDavClient  │ │NasAudioHandler│ │  BackgroundSvc  │              │
│  │  Interface   │ │(audio_service)│ │ (MethodChannel) │              │
│  └──────────────┘ └──────────────┘ └─────────────────┘              │
│  ┌──────────────┐ ┌──────────────┐                                  │
│  │AudioSource-  │ │ LogBuffer    │                                  │
│  │  Builder     │ │              │                                  │
│  └──────────────┘ └──────────────┘                                  │
│       │                                              │               │
├───────┼──────────────────────────────────────────────┤               │
│                       Data Layer                                     │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────────┐             │
│  │  WebDAV API  │ │  SQLite DB   │ │SecureStorage     │             │
│  │  (NAS 远端)  │ │  (本地存储)  │ │SharedPrefs       │             │
│  └──────────────┘ └──────────────┘ └──────────────────┘             │
└──────────────────────────────────────────────────────────────────────┘
```

**数据流：** 用户操作 → Widget → Riverpod Provider → Domain Service → Service/DAO → Data Source → Provider 状态更新 → UI 重建

**DI 桥接：** 跨模块 Provider 通过 `shared/di/providers.dart` 统一导出，消除 feature 之间的循环依赖。
...
**Domain 层（纯 Dart，零 Flutter 依赖）：**
- `ConnectionService` — 连接 CRUD 服务（save 原子操作 + rollback、update、delete 级联清理、setActive 事务切换）
- `ConnectionValidator` — 表单验证纯函数（validateUrl、validateRequired、validateBasePath、validateDdnsHostname）
- `StorageUtils` — 安全存储超时包装（safeStorageRead / safeStorageWrite / safeStorageDelete，5 秒超时）
...
- `connectionDaoProvider` — ConnectionDao 实例
- `connectionServiceProvider` — ConnectionService 实例
- `webDavClientProvider` — WebDavClient 实例
- `secureStorageProvider` — FlutterSecureStorage 实例
...
**Domain 层（纯 Dart，零 Flutter 依赖）：**
- `DirectoryService` — 目录加载、内存缓存、排序。依赖注入 WebDavClientInterface + ISecurePasswordReader + CachePolicy
- `CachePolicy` — TTL 过期 + LRU 驱逐的缓存策略（默认 5 分钟 TTL，上限 50 条）
- `NavigationStackNotifier` — 导航历史栈（push/pop/popTo），始终保留根路径 `/`
- `SortOption` + `SortOptionNotifier` — 排序选项枚举与持久化管理
...
- **目录缓存** — 已加载的目录内容缓存在内存中，TTL 5 分钟 + LRU 驱逐；下拉刷新可清除指定路径缓存
...
- `directoryCacheProvider` — DirectoryService 实例
- `clearDirectoryCacheProvider` — 清除目录缓存
...
**数据模型：** `PlayQueue` — 包含 files（音频文件列表）、currentIndex、startPositionMs、playMode。`PlayMode` 枚举：sequential / repeatOne / repeatAll / shuffle。队列支持 Fisher-Yates 洗牌序列持久化，重启后保留随机顺序。

**Domain 层（纯 Dart，零 Flutter 依赖）：**
- `PlaybackOrchestrator` — 核心播放编排器，协调队列导航、音频加载、进度持久化、监听器管理。所有依赖通过构造函数注入
- `SerializedRequestGate` — 序列化异步请求，确保并发 load 操作安全有序，带 20 秒超时防护
- `PlayMode` + `nextIndex` / `previousIndex` — 播放模式枚举与队列导航纯函数
- `SeekUtils` — seek 操作纯函数（clampSeek / skipForward / skipBackward）
- `SpeedManager` — 速度/步长读取纯函数
- `MediaControl` — 耳机按钮映射、标题提取、时间格式化纯函数
- `BackgroundPlaybackConfig` / `BackgroundPlaybackNotifier` — 后台播放状态机（纯逻辑 StateNotifier）
...
- **队列管理** — 底部弹出队列列表，可查看完整播放列表并点击跳转到任意曲目；支持删除队列中的单条曲目
...
- `playbackOrchestratorProvider` — PlaybackOrchestrator 实例
- `playModeProvider` / `nextPlayModeProvider` — 当前播放模式 / 切换模式
...
- `removeTrackFromQueueProvider` — 从队列中删除曲目
- `backgroundPlaybackProvider` — 后台播放状态机
- `startProcessingListenerProvider` / `cancelProcessingListenerProvider` — 处理状态监听器
- `restoreStartupProgressProvider` — 启动进度恢复
- `reconnectPlaybackListenersProvider` / `cancelPlaybackSubscriptionsProvider` — 播放监听器管理

### 4.4 Timer（定时停止）

纯逻辑定时器状态机，无 Flutter 依赖。

**数据模型：** `TimerState` — 包含 mode（duration / afterCurrent / paused）、endTime、startedAt、remainingMs。`TimerMode` 枚举。

**Domain 层（纯 Dart）：**
- `TimerService` — 定时器状态机（startDuration / startAfterCurrent / cancel / checkExpired / pause / resume / formatRemaining）
- 支持暂停/恢复功能（TMR-03），暂停时保存剩余毫秒数，恢复时重新计算 endTime
...
- `timerStateProvider` / `TimerStateNotifier` — 当前定时器状态
- `timerActiveProvider` / `timerModeProvider` — 派生状态
...
- `lastCustomTimerMinutesProvider` / `setLastCustomTimerMinutesProvider` — 自定义时长记忆
...
**Domain 层（纯 Dart）：**
- `ProgressService` — 进度持久化编排（saveProgress 委托 ProgressDao.upsert）、恢复对话框状态机（showResumeDialog / tickCountdown）
- `ProgressPolicy` — 进度过滤纯函数（shouldSave: position >= 5s；shouldClear: position > duration - 10s 且 duration > 10s）
- `SaveTrigger` 枚举 — 5 种保存触发点（periodic / pause / skipNext / skipPrev / complete）
- `ResumeDialogState` — 恢复对话框不可变状态（progress + countdownSeconds）
...
- `progressResumeProvider` / `ProgressResumeNotifier` — 恢复对话框状态管理
...
**Domain 层（纯 Dart）：**
- `SettingsService` — 设置读写纯逻辑（getThemeMode / setThemeMode / getDefaultSpeed / setDefaultSpeed / getSeekStep / setSeekStep）
...
**Domain 层（纯 Dart）：**
- `PlaylistService` — 播放单 CRUD + 曲目去重 + JSON 导入/导出
...
- **导入/导出** — JSON 格式导出播放单、从 JSON 导入播放单（含去重）
...
- `createPlaylistProvider` / `deletePlaylistProvider` / `updatePlaylistProvider` — 播放单变更
- `addTracksToPlaylistProvider` / `removeTracksFromPlaylistProvider` — 曲目变更
- `reorderPlaylistTrackProvider` — 曲目排序
- `exportPlaylistProvider` / `importPlaylistProvider` — 导入/导出
...
│       ├── storage_utils.dart      # 安全存储超时包装（read/write/delete）
│       └── log_buffer.dart         # 运行时日志环形缓冲区
├── features/                       # 功能模块（每个模块包含 domain/ 子目录）
│   ├── home/
│   │   └── home_screen.dart             # 主页 Tab 导航（播放单 + 文件浏览）
│   ├── connection/
│   │   ├── connection_provider.dart     # Riverpod Provider
│   │   ├── connection_screen.dart       # 添加连接表单
│   │   ├── connection_list_screen.dart  # 连接列表（滑动操作）
│   │   ├── connection_edit_screen.dart  # 编辑连接
│   │   ├── domain/
│   │   │   ├── connection_service.dart  # 连接 CRUD 纯逻辑服务
│   │   │   └── connection_validator.dart # 表单验证纯函数
│   │   └── widgets/
...
│   │   ├── domain/
│   │   │   ├── directory_service.dart # 目录加载 + 缓存 + 排序服务
│   │   │   ├── cache_policy.dart      # TTL + LRU 缓存策略
│   │   │   └── navigation_stack.dart  # 导航历史栈
│   │   └── widgets/
...
│   │   ├── background_playback.dart # 后台播放状态机（兼容旧导入）
│   │   ├── media_control_model.dart # 媒体控制模型（兼容旧导入）
│   │   ├── domain/
│   │   │   ├── playback_orchestrator.dart # 核心播放编排器（零 Riverpod 依赖）
│   │   │   ├── request_gate.dart          # SerializedRequestGate + PlayerLoadState
│   │   │   ├── play_mode.dart             # PlayMode 枚举 + nextIndex/previousIndex
│   │   │   ├── seek_utils.dart            # seek 纯函数（clamp/skip）
│   │   │   ├── speed_manager.dart         # 速度/步长读取纯函数
│   │   │   ├── media_control.dart         # 耳机映射 + 标题提取 + 时间格式化
│   │   │   └── background_playback.dart   # 后台播放状态机（StateNotifier）
│   │   └── widgets/
...
│   │   ├── domain/
│   │   │   └── playlist_service.dart    # 播放单 CRUD + 去重 + 导入/导出
│   │   └── widgets/
...
│   │   └── domain/
│   │       ├── progress_service.dart # 进度持久化 + 恢复对话框状态机
│   │       └── progress_policy.dart  # shouldSave/shouldClear 纯函数
│   └── settings/
...
│       └── domain/
│           └── settings_service.dart # 设置读写纯逻辑
├── shared/
│   ├── di/
│   │   └── providers.dart          # 跨模块 Provider 统一导出（消除循环依赖）
│   └── models/
│       ├── connection_config.dart  # 连接配置模型
│       ├── nas_file.dart           # 文件/目录模型
│       ├── play_progress.dart      # 播放进度模型
│       ├── play_queue.dart         # 播放队列模型（含 Fisher-Yates 洗牌 + 序列化）
│       ├── playlist.dart           # 播放单模型（Playlist / PlaylistTrack）
│       └── ...
└── main.dart                       # 应用入口 + ProviderScope 覆盖 + 路由配置
```
...
1. **Domain 层零依赖** — 每个 feature 的 `domain/` 子目录包含纯 Dart 逻辑，不依赖 Flutter widget、Riverpod 或平台通道。所有外部依赖通过构造函数注入，可直接单元测试
2. **Contract 解耦** — `core/contracts/` 定义抽象接口（IAudioPlayer、IAudioHandler、IConnectionDao 等），Domain/Provider 层依赖接口而非具体实现，测试时注入 Fake/Mock
3. **纯逻辑层可测试** — TimerService、PlaybackOrchestrator、PlayQueue 导航、seek 计算、进度过滤规则、缓存策略、表单验证均为纯函数或纯逻辑类，不依赖 Flutter 或平台通道
4. **Provider 覆盖** — 基础设施（AudioPlayer、SharedPreferences、AudioHandler）通过 `ProviderScope.overrides` 注入，方便测试替换
5. **密码安全** — 明文密码仅存储在 flutter_secure_storage 中，数据库中仅保存引用 key；传输时通过 HTTPS 或 Basic Auth Header；安全存储操作带 5 秒超时防护
6. **加载序列化** — 通过 `SerializedRequestGate` 确保对共享 AudioPlayer 的并发操作安全有序，带 20 秒超时防止永久卡死
7. **队列持久化** — 播放队列序列化到 SharedPreferences（含 Fisher-Yates 洗牌顺序），App 重启后自动恢复；连接变更时保留队列但跳过预加载
8. **跨模块 DI 桥接** — `shared/di/providers.dart` 是唯一允许导入多个 feature 的文件，统一导出跨模块 Provider，消除 feature 之间的循环依赖
9. **进度智能过滤** — 位置 < 5 秒不保存（视为未开始）；位置 > duration - 10 秒清除记录（视为已听完）；短于 10 秒的文件不自动清除
